...
public contnue:string="div[class='mtn-stack-h form-bottom-control'] div[class='mtn-btn-group']"
public prcdtpay:string="div[class='payment-method payment-method-npg _active'] div[class='payment-method-content'] span"//"#checkout-payment-method-load > div > div > div.payment-method.payment-method-npg._active > div.payment-method-content > div.actions-toolbar > div > button"
...
public orderNumber:string ="#maincontent > div.columns.mf-initial > div > div.content-wrapper.wrapper.mf-initial > section > div > div > div.row.mf-initial > div.col-md-5.su-left-block.mf-initial > div > div.font-13.txt-color-b-7.b-tns.mt-2.mf-initial > strong"
public ordersBtn: string  ="li.link.my-orders > a"
public actionBtn:string = "#html-body > div.page-wrapper.mf-initial > div.main-header.cart-header-label.mf-initial > header > div.header.content.mf-initial > ul > li > span > button"//".cus tomer-first-name"
//methods
clickOncartIoc(){
...
enterEmailD(){
    ...
    cy.get(this.prcdtpay).click({force:true})
       ...
       cy.on('uncaught:exception', (e) => {
        ...
        cy.get(this.prcdtpay).click({force:true})
             ...
             // cy.get("#mat-expansion-panel-header-0 > span").click({force:true})
           ...
           cy.wait(1000)
           cy.visit('https://mymtnngnxtgen-shop-staging.mtnnigeria.net/checkout/onepage/success/npg')
    ...
    cy.get(this.orderNumber).then(($order)=>{
        const OrderNo = $order.text()
    })
    Basepage.clickElementForce(this.actionBtn)   
    Basepage.clickElementForce(this.ordersBtn)  
    cy.get('.font-24.black-color.hd-m-all').should('contain.text','My orders')  
    cy.get("#processing-tab").click()
    cy.get("#processing > div:nth-child(1) > div.new-ord-top.mf-initial > div > div.col-lg-3.mf-initial > div > div > div > a").click({force:true})
    cy.wait(10000)
    }
}
